  ...
  <div class="min-h-screen flex">
    ...
    >
      <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center" :class="sidebarCollapsed ? 'justify-center' : 'justify-between'">
        <NuxtLink v-if="!sidebarCollapsed" to="/" class="block flex-1">
          <img 
      ...
      <nav class="flex-1 overflow-y-auto p-2 space-y-1">
        ...
        >
          ...
          <span v-if="!sidebarCollapsed">Roadmap</span>
        </NuxtLink>
        ...
        <NuxtLink 
          ...
          :class="sidebarCollapsed ? 'justify-center' : ''"
          active-class="bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-medium"
          :title="sidebarCollapsed ? 'Reports 2.0' : ''"
        >
          <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" />
          </svg>
          <span v-if="!sidebarCollapsed">Reports 2.0</span>
          <span v-if="!sidebarCollapsed" class="ml-auto text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-0.5 rounded">New</span>
        </NuxtLink>
      ...
      <ClientOnly>
        <div class="p-2 border-t border-gray-200 dark:border-gray-700" :class="sidebarCollapsed ? 'flex flex-col items-center' : 'p-4'">
          ...
          <div v-else class="w-10 h-10 rounded-full bg-primary-600 flex items-center justify-center text-white font-semibold mb-2" :title="authStore.user?.full_name || 'User'">
            {{ authStore.user?.full_name?.charAt(0)?.toUpperCase() || '?' }}
          </div>
          
          <button v-if="!sidebarCollapsed" @click="toggleDarkMode" class="w-full btn-secondary mb-2 text-sm">
            <svg v-if="colorMode.value === 'dark'" class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button v-else @click="toggleDarkMode" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 mb-1" :title="colorMode.value === 'dark' ? 'Light Mode' : 'Dark Mode'">
          ...
          <button v-if="!sidebarCollapsed" @click="showPasswordModal = true" class="w-full btn-secondary mb-2 text-sm">
            <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
            </svg>
            Change Password
          </button>
          <button v-else @click="showPasswordModal = true" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400 mb-1" title="Change Password">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
            </svg>
          </button>
          
          <button v-if="!sidebarCollapsed" @click="handleLogout" class="w-full btn-secondary text-sm">
            <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
            Logout
          </button>
          <button v-else @click="handleLogout" class="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 dark:text-gray-400" title="Logout">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
            </svg>
          </button>
